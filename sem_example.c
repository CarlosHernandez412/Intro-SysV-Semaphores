 ...
 *
 *   $ gcc -o sem_example sem_example.c
 *   $ ./sem_example
 *
 ...
 *   subtracted from the value of the semaphore. For example, if the value of
 *   the semaphore is 5 and sem_op is -1, 5 >= |-1| so it would substract 1 to
 *   make the new semaphore value 4.
 *
 * + If sem_op is a negative int and the value of the semaphore is less than 
 *   the absolute value of sem_op, then the execution of test immediately 
 *   stops if IPC_NOWAIT is true, or blocks until the semaphore's value becomes 
 *   greater than the absolute value of sem_op if false. For example, if the
 *   semaphore is currently 0 and sem_op is -1, and IPC_NOWAIT is not true, 
 *   0 < |-1| so the process would block until the semaphore is 1 or higher.
 *
 * This means there are two cases when IPC_NOWAIT is false where processes
 * would block on calling semop() with the sembuf structure:
 *   1) sem_op is 0 and the semaphore is NOT currently 0. Blocks until the
 *      semaphore is 0.
 *   2) sem_op is -n and the semaphore value is less than n. Blocks until
 *      the semaphore value is n or higher.
 *
 ...
 * resource is free if the value of the semaphore is 0 (e.g. block requesting 
 * processes when the semaphore is not 0).
 *
 ...
 * Two semaphore operations are specified. The first is 0, which means the
 ...
 * The process calls semop to run the operations, passing semaphore ID, the 
 ...
 *
 * Sample output of two instances of this application:
 *
...
int main (int argc, char **argv) 
{
   ...
   /* Create the variables to hold the semaphore operations. 
    * If you will be changing sem_op, sem_num, or sem_flg during execution, do 
    * NOT declare this in global space, and instead have a copy of the sembuf 
    * variables for each process or thread. 
    * If you will not be changing the value of any of the member variables 
    * during execution, it's okay to have in global space, although a 
    * misbehaving process or thread could unexpectedly change values. Be 
    * careful to review your code to make sure it's not doing that */
   ...
   /* Query the value of the semaphore */
   ...
   while(loop) {
        ...
        semop(semid, grab, 2); /* perform the two operations */

        /* CRITICAL SECTION ENTRY */
        printf("%d has the semaphore\n", mypid);

        /* this is how the one that got the semaphore can check to see if 
         * anyone is waiting on zero */
        n_waiting = semctl(semid, 0, GETZCNT);
        if (n_waiting) printf("%d process blocking on zero.\n", n_waiting);

        /* this is how to check if anyone is waiting on sem to be incremented */
        n_waiting = semctl(semid, 0, GETNCNT);
        if (n_waiting) printf("waiting for pos value: %d\n",n_waiting);
        ...
        sleep(rand() % 3); /* busy work, take a nap for 0-2 seconds */
    ...
    /* After the loop, clean up from the child process and delete the
...
void sigint_handler(int sig)
{
  ...
  /* PARENT HANDLER*/
     ...
     /* clean up IPC objects */
     if (semctl(semid, 0, IPC_RMID) < 0) perror("IPC_RMID error:");
     else write(1,"semaphore removed\n",19);
     exit(0);
  }
}
